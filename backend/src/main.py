...
# /app/ serves the single pillow SPA. Registered BEFORE the static mount so
# the explicit route wins at /app/ (the mount handles nested asset paths).
# no-cache on the HTML so users always get the latest markup between deploys;
# the mount below serves CSS/JS/vendor assets with default caching.
if _FRONTEND_DIR is not None:
    @app.get("/app/", include_in_schema=False)
    @app.get("/app", include_in_schema=False)
    async def _serve_ui():
        resp = FileResponse(_FRONTEND_DIR / "index.html", media_type="text/html")
